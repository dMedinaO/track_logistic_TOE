<?php
	...
	$query = "select SUM(reparto.paquetes_entregados) as cantidad from reparto";
	
	if ($periode == 1){#semana
		$query = $query." where WEEK(fecha_reparto) = WEEK(NOW())";
	}elseif ($periode == 2) {
		$query = $query." where DATE_FORMAT(fecha_reparto, '%m') = DATE_FORMAT(NOW(), '%m')";
	}
	...
	$query = "select DISTINCT(COUNT(*)) as cantidad from reparto_chofer join reparto on (reparto.idreparto = reparto_chofer.reparto ) where reparto_chofer.status_reparto = 'FINALIZADO'";

	if ($periode == 1){#semana
		$query = $query." AND WEEK(fecha_reparto) = WEEK(NOW())";
	}elseif ($periode == 2) {
		$query = $query." AND DATE_FORMAT(fecha_reparto, '%m') = DATE_FORMAT(NOW(), '%m')";
	}
	...
	$query = "select DISTINCT(COUNT(*)) as cantidad from reparto_chofer join reparto on (reparto.idreparto = reparto_chofer.reparto ) where reparto_chofer.status_reparto ='PENDIENTE'";
