<?php
	...
	$query_basic = "select *, DATE_FORMAT(reparto.fecha_reparto, '%Y-%m-%d') as fecha_trunc_reparto from reparto join reparto_chofer on (reparto.idreparto = reparto_chofer.reparto) join chofer on (chofer.idchofer = reparto_chofer.chofer) join comuna on (comuna.idcomuna = reparto.comuna) where chofer.client=$client ";
	...
	if ($status_reparto !=3 and $chofer != -1){#todos los filtros
		$query_basic = $query_basic." and ".$add_status. " and ".$add_periode." and ".$add_chofer. " order by chofer.nombre_chofer DESC";
	}

	if ($status_reparto ==3 and $chofer == -1){#no filtros todos estados todos reparto
		$query_basic = $query_basic." and ".$add_periode. " order by chofer.nombre_chofer DESC";
	}

	if ($status_reparto !=3 and $chofer == -1){#filtro por estado de reparto y todos los choferes
		$query_basic = $query_basic." and ".$add_status." and ".$add_periode. " order by chofer.nombre_chofer DESC";
	}

	if ($status_reparto ==3 and $chofer != -1){#todos los estados y un chofer
		$query_basic = $query_basic." and ".$add_chofer." and ".$add_periode. " order by chofer.nombre_chofer DESC";
	}

	if ($status_reparto !=3 and $chofer != -1){#chofer y status
		$query_basic = $query_basic." and ".$add_chofer." and ".$add_status." and ".$add_periode. " order by chofer.nombre_chofer DESC";
	}

	if ($status_reparto !=3 and $chofer == -1){#periode y status
		$query_basic = $query_basic." and ".$add_periode." and ".$add_status. " order by chofer.nombre_chofer DESC";
	}
